...
const {notes} = require("../../database/config");
...
/*
//new apis u are going to create
http://34.215.133.182/api/<app_id>/edit // interface for editing quiz in iframe need to pass token along with this url

http://34.215.133.182/api/<app_id>/attend // interface for attending quiz in iframe need to pass token along with this url
*/
infceRouter.get("/:app_id/edit/:token" , verify_access_tokens_admin_user , ( req , res )=>{
  ...
  qtnr.findOne({_id:app_id} , (error , qtnrObject)=>{
    ...
    if ( app_type == 0 ) application_type = "survey-creation";
    else application_type = "quiz-creation";
