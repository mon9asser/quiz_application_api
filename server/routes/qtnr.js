...
qtnrRouters.post("/upload/animage"  , question_answer_images.single("media_field") , (req, res) => {
    ...
    Jimp.read(fileIs, (err, Image) => {
        if (err) {
          res.send({ status_code : 0 , error : ersur , message : 'Failed'  });
          return false ;
        };
  ...
  var userType = req.is_creator;
    ...
    req.body.app_settings = {
               ...
               expiration : {
                 ...
                 expire_warning : "This quiz will expire in next {{ date | long }} after {{ hour_counts }} hour(s)" ,
                 expire_message : "This Quiz is Expired from {{ time_ago }}  hour(s) ago !"
               } ,
              //  theme_style : [] ,
...
qtnrRouters.post("/:app_id/truncate/attendee/data/object" , (req , res) => {
  var user_id = req.body.user_id ;
  var app_id = req.params.app_id ;

  // ==> Delete From Online Report
